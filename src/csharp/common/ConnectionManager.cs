...
namespace Mas.Infrastructure.Common
{
            ...
            if (sturdyRef.StartsWith("capnp://"))
            {
                ...
                if (addressPort.Length > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(srToken))
                    {
                        var restorer = con.GetMain<Rpc.Persistence.IRestorer<string>>();
                        var cap = await restorer.Restore(srToken, null);
                        var castCap = cap.Cast<TRemoteInterface>(true);
                        var lc = new LockableCap<TRemoteInterface>(con, castCap);
                        _ConnectionsL.Add(lc);
                        return lc;
                    }
                    else
                    {
                        var bootstrap = con.GetMain<TRemoteInterface>();
                        var lc = new LockableCap<TRemoteInterface>(con, bootstrap);
                        _ConnectionsL.Add(lc);
                        return lc;
                    }
                }
    ...
    {
        object _lock = new();
        bool _capBorrowed = false;

        TcpRpcClient _con;
        TCapnpInterface _cap;

        public LockableCap(TcpRpcClient con, TCapnpInterface cap)
        {
            _con = con;
            _cap = cap;
        }

        public TCapnpInterface BorrowCap()
        {
            Monitor.Enter(_lock, ref _capBorrowed);
            return _cap;
        }

        public void ReturnCap(TCapnpInterface cap)
        {
            if (_cap == cap && _capBorrowed)
            {
                _capBorrowed = false;
                Monitor.Exit(_lock);
            }
