...
vector<string> Tools::splitString(const string& s,
  ...
  for (char c : s) {
    if (splitElements.find(c) == string::npos || delimiterLevel > 0) {
      int levelInc = 0;
      if (!tokenDelimiters.first.empty() && tokenDelimiters.first.find(c) != string::npos) levelInc = 1;
...
int Tools::createRandomNumber() {
  ...
  while (id == 0) {
    ...
    if (lastTime != t) srand(static_cast<unsigned int>(t));
    id = rand();
    lastTime = t;
  }
  return id;
...
double Tools::hourlyT(double tmin, double tmax, int h, int sunrise_h) {
  ...
  if (h < sunrise_h) H_1 = h + 10.0;
  else if (h > 14.0) H_1 = h - 14.0;

  if (h < sunrise_h || h > 14.0) hourly_T = tavg + amp_ * (cos(M_PI * H_1 / (10.0 + sunrise_h)));
  else hourly_T = tavg - amp_ * (cos(M_PI * (h - sunrise_h) / (14.0 - sunrise_h)));
...
HistogramData Tools::histogramDataByStepSize(const vector<double> &ys,
  ...
  res.xs.resize(n);
  for (int i = 0; i < n; i++) res.xs[i] = minY + (i * step + (step / 2));
  //cout << "xs: " << xs << endl;

  res.borders.resize(n + 1);
  for (int i = 0; i < n + 1; i++) res.borders[i] = minY + (i * step);
  ...
  for (double y : ys) {
    ...
    res.classes[int(std::ceil((shift + y) / step))]++;
  }
