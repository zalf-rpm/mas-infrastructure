...
bool Tools::printPossibleErrors(const Errors& es, bool includeWarnings) {
  if (es.failure()) for (auto e : es.errors) std::cerr << e << std::endl;

  if (includeWarnings) for (auto w : es.warnings) std::cerr << w << std::endl;
...
EResult<string> Tools::readFile(string path) {
  ...
  ifs.open(path);
  if (ifs.good()) {
    for (string line; getline(ifs, line);) res.result += line;
  } else res.errors.push_back(string("Couldn't open file: '") + path + "'");
...
bool Tools::isAbsolutePath(const std::string& path) {
  ...
  //unix absolute path
  if (found == 0 && path.at(0) == '/') return true;
...
std::string Tools::winStringSystemCodepageToutf8(const std::string& codepage_str) {
  ...
  MultiByteToWideChar(CP_ACP, 0, codepage_str.c_str(), cplen, &utf16_str[0], size);

  int utf16Len = (int)utf16_str.length();
  int utf8_size = WideCharToMultiByte(CP_UTF8, 0, utf16_str.c_str(), utf16Len, nullptr, 0, nullptr, nullptr);
  std::string utf8_str(utf8_size, '\0');
  WideCharToMultiByte(CP_UTF8, 0, utf16_str.c_str(), utf16Len, &utf8_str[0], utf8_size, nullptr, nullptr);
...
std::string Tools::replaceEnvVars(std::string path) {
  ...
  while (startPos != std::string::npos) {
    ...
    if (endPos != std::string::npos) {
      ...
      } else startPos = path.find(startToken, endPos + 1);
    }
  }
