...
{
	if (restorer)
		_restorer = kj::Own<Restorer>(restorer, _disposer);
	else
		_restorer = kj::heap<Restorer>();
}
			...
			if (!addressPort.size() == 0)
			{
				...
				else
				{
					return ioc.provider->getNetwork().parseAddress(addressPort).then(
						...
						[readerOpts, this, KJ_MVCAP(addressPort), KJ_MVCAP(srTokenBase64)](kj::Own<kj::AsyncIoStream> &&stream)
						{
