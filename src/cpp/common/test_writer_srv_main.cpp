...
class TestWriterSrvMain
{
public:
  TestWriterSrvMain(kj::ProcessContext &context) : context(context), ioContext(kj::setupAsyncIo()) {}

  kj::MainBuilder::Validity setName(kj::StringPtr n) { name = n; return true; }

  kj::MainBuilder::Validity setOutSr(kj::StringPtr name) { outSr = name; return true; }
  ...
  kj::MainBuilder::Validity startWriter()
  {
    ...
    auto outp = conMan.tryConnectB(ioContext, outSr.cStr()).castAs<Y>();
    ...
    KJ_LOG(INFO, "TestWriterSrvMain::startWriter: stopping writer");
    return true;
  }
