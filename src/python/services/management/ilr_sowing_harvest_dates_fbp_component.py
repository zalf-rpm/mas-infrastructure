...
def create_seed_harvest_geoGrid_interpolator_and_read_data(path_to_csv_file, wgs84_crs, target_crs, ilr_seed_harvest_data):
    ...
    with open(path_to_csv_file) as _:
        ...
        for row in reader:
            ...
            ilr_seed_harvest_data[crop_id]["data"][cs]["sowing-date"] = {"year": 0, "month": sd.month, "day": sd.day} #"0000-{:02d}-{:02d}".format(sd.month, sd.day)

            esdoy = int(float(row[8]))
            ilr_seed_harvest_data[crop_id]["data"][cs]["earliest-sowing-doy"] = esdoy
            esd = base_date + timedelta(days = esdoy - 1)
            ilr_seed_harvest_data[crop_id]["data"][cs]["earliest-sowing-date"] = {"year": 0, "month": esd.month, "day": esd.day} #"0000-{:02d}-{:02d}".format(esd.month, esd.day)

            lsdoy = int(float(row[9]))
            ilr_seed_harvest_data[crop_id]["data"][cs]["latest-sowing-doy"] = lsdoy
            lsd = base_date + timedelta(days = lsdoy - 1)
            ilr_seed_harvest_data[crop_id]["data"][cs]["latest-sowing-date"] = {"year": 0, "month": lsd.month, "day": lsd.day} #"0000-{:02d}-{:02d}".format(lsd.month, lsd.day)
            ...
            ilr_seed_harvest_data[crop_id]["data"][cs]["harvest-date"] = {"year": 0, "month": hd.month, "day": hd.day} #"000{}-{:02d}-{:02d}".format(digit, hd.month, hd.day)
            ...
            ilr_seed_harvest_data[crop_id]["data"][cs]["earliest-harvest-date"] = {"year": digit, "month": ehd.month, "day": ehd.day} #"000{}-{:02d}-{:02d}".format(digit, ehd.month, ehd.day)

            lhdoy = int(float(row[11]))
            ilr_seed_harvest_data[crop_id]["data"][cs]["latest-harvest-doy"] = lhdoy
            lhd = base_date + timedelta(days = lhdoy - 1)
            ilr_seed_harvest_data[crop_id]["data"][cs]["latest-harvest-date"] = {"year": digit, "month": lhd.month, "day": lhd.day} #"000{}-{:02d}-{:02d}".format(digit, lhd.month, lhd.day)
...
ilr_path_template = str(PATH_TO_REPO) + "/data/management/ilr_seed_harvest_doys_germany/ILR_SEED_HARVEST_doys_{crop_id}.csv"
...
try:
    for crop_id in config["crop_ids"].split(","):
        ...
        create_seed_harvest_geoGrid_interpolator_and_read_data(path_harvest, wgs84_crs, utm32n_crs, ilr_seed_harvest_data)
except IOError:
