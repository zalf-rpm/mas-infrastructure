...
def start_monica_capnp_proxy():
    p = Popen(["monica-capnp-proxy", "-p", str(MONICA_SERVICE_PROXY_PORT), "-t", "3"],
              cwd=PATH_TO_MONICA_CAPNP_PROXY_EXECUTABLE)
    time.sleep(0.1)
    yield
    p.terminate()
...
def monica_cap(start_monica_capnp_proxy):
    monica = capnp.TwoPartyClient("localhost:" + str(MONICA_SERVICE_PROXY_PORT)).bootstrap().cast_as(
...
def time_series_cap(start_time_series_service):
    csv_time_series = capnp.TwoPartyClient("localhost:" + str(TIME_SERIES_SERVICE_PORT)).bootstrap().cast_as(
...
def soil_service_cap(start_soil_service):
    soil_service = capnp.TwoPartyClient("localhost:" + str(SOIL_SERVICE_PORT)).bootstrap().cast_as(
        soil_capnp.Service)
    return soil_service
