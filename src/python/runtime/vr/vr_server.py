...
class GK5Cell2LatLon(vr.Rt.Cell2Coord2Cell.Server):

    def __init__(self, nrows, ncols, xllcorner, yllcorner, cellsize):
        self._nrows = nrows
        self._ncols = ncols
        self._xll_center = xllcorner + cellsize/2
        self._yll_center = yllcorner + cellsize/2
        self._cellsize = cellsize
        self._gk5 = Proj(init="epsg:31469") 
        self._wgs84 = Proj(init="epsg:4326")
        self._transformer = Transformer.from_proj(self._wgs84, self._gk5) 

    def rowcol_to_center_latlon(self, row, col):
        "rowcol is 1-indexed"

        h = self._yll_center + (self._nrows - row) * self._cellsize
        r = self._xll_center + col * self._cellsize
        lon, lat = self._transformer.transform(r, h)
        return (lat, lon)

    def cell2Coord_context(self, context): # cell2Coord @0 Cell2D -> Geo.Coord;
        ps = context.params
        lat, lon = self.rowcol_to_center_latlon(ps.y, ps.x)
        return {"latlon": {"lat": lat, "lon": lon}}

    def center_latlon_to_rowcol(self, lat, lon):
        "rowcol is 1-indexed"

        r, h = self._transformer.transform(lon, lat)
        row = int(self._nrows - ((h - self._yll_center) / self._cellsize)) 
        col = int((r - self._xll_center) / self._cellsize)
        return (row, col)

    def coord2Cell_context(self, context): # coord2Cell @1 Geo.Coord -> Cell2D;
        ps = context.params
        row, col = self.center_latlon_to_rowcol(ps.latlon.lat, ps.latlon.lon)
        return {"x": col, "y": row}}




class Unregister(c.Common.Registry.Unregister.Server):

    def __init__(self, unregAction):
        self._unregAction = unregAction
        self._already_unregistered = False

    def __del__(self):
        if not self._already_unregistered:
            self._unregAction()

    def unregister(self, **kwargs): # unregister @0 ();
        self._unregAction()
        self._already_unregistered = True


class VR(vr.RT.VR.Server):
     
    def __init__(self, resolution, bounds):
        self._resolution = resolution
        self._bounds = bounds
        self._data = {}
        self._data_unreg_actions = {}
        self._models = {}
        self._mappings = {}
        self._mappings_unreg_actions = {}

    def params_context(self, context): # params @0 () -> VRParams;
        res = context.results
        res.resolution = self._resolution
        res.regionParams = {"bounds": self._bounds}

    def registerCoordMapping(self, coordType, func, **kwargs): # registerCoordMapping @4 (coordType :Geo.CoordType, func :Cell2Coord2Cell) -> (unregister :Common.Registry.Unregister);
        self._mappings[coordType] = func
        unreg = Unregister(lambda: self._mappings.pop(coordType, None))
        self._mappings_unreg_actions[coordType] = unreg
        return unreg

    def registerDataService(self, id, data, **kwargs): # registerDataService @1 (id :Text, data :DataService) -> (unregister :Common.Registry.Unregister);
        self._data[id] = data
        unreg = Unregister(lambda: self._data.pop(id, None))
        self._data_unreg_actions[id] = unreg
        return unreg

    def startCachingData_context(self, context): # startCachingData @1 () -> (stop :Common.Callback);
        cell_to_latlon = self._mappings.get("latlon", None)
        if not cell_to_latlon:
            return
        for row in range(self._bounds["br"]["y"] - self._bounds["tl"]["y"] + 1):
            for col in range(self._bounds["br"]["x"] - self._bounds["tl"]["x"] + 1):
                lat_lon = cell_to_latlon.cell2Coord({"x": col, "y": row}).wait().latlon
                for id, data_cap in self._data.items():
                    data = data_cap.getDataAtCoord(lat_lon).wait().data
                    #store data here


class Factory(vr.RT.Factory.Server):

    def __init__(self):
        self._vrs = []

    def createVR_context(self, context): # createVR @0 VRParams -> (vr :VR);
        ps = context.params
        bs = ps.regionParams.bounds
        vr = VR(ps.resolution, {
            "tl": {"x": bs.tl.x, "y": bs.tl.y},
            "br": {"x": bs.br.x, "y": bs.br.y}
        })
        self._vrs.append(vr)
        context.results.vr = vr
...
def main():
    ...
    server = capnp.TwoPartyServer("*:" + config["port"], bootstrap=Factory())
    server.run_forever()
