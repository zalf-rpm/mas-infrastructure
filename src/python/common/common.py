...
class Restorer(persistence_capnp.Restorer.Server):
    ...
    def init_port_from_container(self):
        ...
        try:
            return self.storage_container.getEntry(key="port").send().getEntry.getValue().send().then(resp)
    ...
    def init_vat_id_from_container(self):
        ...
        vat_sign_pk_req = self.storage_container.getEntry(key="vatSignPK").send()
        vat_sign_sk_req = self.storage_container.getEntry(key="vatSignSK").send()
        ...
        try:
            proms.append(vat_sign_pk_req.send().then(lambda resp: pk_sk_resp("vatSignPK", self._sign_pk, resp)))
            proms.append(vat_sign_sk_req.send().then(lambda resp: pk_sk_resp("vatSignSK", self._sign_sk, resp)))
    ...
    def sturdy_ref_str(self, sr_token=None):
        ...
        return "capnp://{vat_id}@{host}:{port}{sr_token}".format(
            vat_id=base64.urlsafe_b64encode(self._sign_pk).decode("utf-8"), 
            host=self.host, 
            port=self.port, 
