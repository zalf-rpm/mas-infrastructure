...
kj::Promise<void> CapHolderImpl::cap(CapContext context) {
  context.getResults().getCap().setAs<capnp::Capability>(_cap);
  return kj::READY_NOW;
}
...
kj::Promise<void> CapHolderListImpl::cap(CapContext context) {
  ...
  auto list = rs.getCap().initAs<capnp::List<rpc::Common::ListEntry<rpc::Common::CapHolder<capnp::AnyPointer>>>>(caps.size());
  int i = 0;
  for (auto& cap : caps) {
    auto entryB = list[i];
