...
from data_services.soil.python import sqlite_soil_data_service as soil_service


def test_getAllAvailableParameters(soil_service_cap):
    params = soil_service_cap.getAllAvailableParameters().wait().params
    assert len(params) == 16
    assert list(map(lambda p: p.which(), params))[:4] == ["size", "ka5SoilType", "sand", "clay"]


def test_checkAvailableParameters(soil_service_cap):
    res = soil_service_cap.checkAvailableParameters(
        mandatory=[{"sand": 0}, {"clay": 0}, {"bulkDensity": 0}, {"organicCarbon": 0}],
        optional=[{"pH": 0}]
    ).wait()
    assert res.failed is False
    assert len(res.mandatory) == 4
    assert list(map(lambda p: p.which(), res.mandatory)) == ["sand", "clay", "bulkDensity", "organicCarbon"]
...
def test_profilesAt(soil_service_cap):
    profiles = soil_service_cap.profilesAt(
