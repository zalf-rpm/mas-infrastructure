...
class Service(soil_data_capnp.Soil.Service.Server):
    ...
    def profilesAt_context(self, context): # profilesAt @0 Geo.Coord -> (profiles :List(Profile));
        pass


    def allProfiles_context(self, context): # allProfiles @1 () -> (profiles :List(Common.Pair(Geo.Coord, List(Common.CapHolder(Profile)))));    
        pass


def main(path_to_sqlite_db, server="*", port=11002):

    config = {
        "port": str(port),
        "server": server,
        "path_to_csv_file": path_to_sqlite_db
    }
